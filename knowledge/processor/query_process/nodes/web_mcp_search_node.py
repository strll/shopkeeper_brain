    ...
    async def _create_execute_web_search(self, validated_rewritten_query: str) -> List[Dict[str, Any]]:
        ...
        mcp_client = MCPServerStreamableHttp(
            ...
            params={
                ...
                "headers": {"Authorization":  f"Bearer {api_key}"}  # 认证权限 api_key
            },
...
if __name__ == '__main__':
    ...
    mcp_search = WebMcpSearchNode()

    # 这里现在能正确接收到 process return 返回的 state 字典了
    result = mcp_search.process(state)

    # 确保刚才保存的 web_search_docs 不是 None
    search_docs = result.get('web_search_docs')
    if search_docs:
        for r in search_docs:
